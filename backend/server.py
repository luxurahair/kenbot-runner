    ...
    if trim_val and ("," in trim_val or "/" in trim_val or len(trim_val) > 30):
        try:
            ...
            if api_key:
                chat = LlmChat(
                    api_key=api_key,
                    session_id=f"vin-trim-{vin[-6:]}",
                    system_message="Tu es un expert en decodage VIN automobile. Tu connais la structure des VINs par manufacturier (positions 4-8 = modele/moteur/trim). Reponds UNIQUEMENT avec le nom du trim exact, rien d'autre. Si tu n'es pas sur, donne ta meilleure estimation.",
                )
                chat.with_model("openai", "gpt-4o")
                prompt = f"VIN: {vin}\nMarque: {specs.get('make','')}\nModele: {specs.get('model','')}\nAnnee: {specs.get('year','')}\nTrims possibles (NHTSA): {trim_val}\nMoteur: {specs.get('engine_cylinders','')}cyl {specs.get('engine_displacement','')}L\nCarburant: {specs.get('fuel_type','')}\n\nQuel est le trim EXACT de ce VIN? Analyse les caracteres 4-8 du VIN pour determiner le trim precis. IMPORTANT: Pour les vehicules vendus au Canada, utilise les noms canadiens (ex: VW Canada = Trendline/Comfortline/Highline au lieu de S/SE/SEL). Le VIN commence par 3VW = Mexique pour marche nord-americain."
